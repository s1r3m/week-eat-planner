...
from week_eat_planner.exceptions import WeekNotFound, WeekForbidden
...
async def get_week_for_update(
    week_snapshot: Annotated[Week, Depends(get_week_by_id)],
    ...
    """
    week = await WeekDAO(write_session).get_week_for_update(week_snapshot)
    if not week:
