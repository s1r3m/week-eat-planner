...
from week_eat_planner.api.schemas import UserShow, WeekModel
...
async def get_weeks(
    user: Annotated[UserShow, Depends(get_current_active_user)],
    session: AsyncSession = Depends(db.get_db),
) -> list[WeekModel]:
    weeks = await WeekDAO(session).get_weeks_by_user(user)
