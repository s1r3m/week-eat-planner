...
async def test_delete_recipe__recipe_with_image__image_deleted_from_storage(
    auth_client_for_created_user, created_recipe_with_image, mocker
):
    # Mock storage client
    mock_storage = mocker.AsyncMock(spec=StorageClient)
    app.dependency_overrides[get_storage_client] = lambda: mock_storage

    try:
        old_image_key = created_recipe_with_image.image_key
        response = await auth_client_for_created_user.delete(
            f'{AppUrl.RECIPES_TPL.format(recipe_id=created_recipe_with_image.id)}'
        )

        assert response.status_code == status.HTTP_204_NO_CONTENT
        mock_storage.delete_file.assert_called_once_with(old_image_key)
    finally:
...
async def test_upload_image__valid_file__image_is_uploaded(auth_client_for_created_user, created_recipe):
    ...
    expected.pop('image_url')
    assert body.pop('image_url')
    assert body == expected

# These tests are autogenerated.
async def test_upload_image__recipe_has_old_image__old_image_is_deleted(
    auth_client_for_created_user, created_recipe_with_image, mocker
):
    # Mock storage client
    mock_storage = mocker.AsyncMock(spec=StorageClient)
    mock_storage.upload_image.return_value = 'new_image_key.jpg'
    app.dependency_overrides[get_storage_client] = lambda: mock_storage

    try:
        old_image_key = created_recipe_with_image.image_key
        files = {'image': ('new.jpg', b'new content', 'image/jpeg')}

        response = await auth_client_for_created_user.patch(
            f'{AppUrl.RECIPES_IMAGE_TPL.format(recipe_id=created_recipe_with_image.id)}',
            files=files,
        )
...
async def test_upload_image__delete_old_image_fails__request_still_succeeds(
    auth_client_for_created_user, created_recipe_with_image, mocker
):
    # Mock storage client
    mock_storage = mocker.AsyncMock(spec=StorageClient)
...
# The end of autogenerated tests.
