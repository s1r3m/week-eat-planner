...
from week_eat_planner.constants import AUTH_LOGIN, AUTH_PREFIX, AUTH_SIGNUP
...
async def user(client, login_data) -> dict[str, str]:
    ...
    await client.post(f'{AUTH_PREFIX}{AUTH_SIGNUP}', json=login_data)
    return login_data
...
async def test_add_user__duplicate_email__conflict_error(client, user):
    response = await client.post(f'{AUTH_PREFIX}{AUTH_SIGNUP}', json=user)
...
async def test_add_user__invalid_email_format__unprocessable_entity_error(client):
    ...
    response = await client.post(f'{AUTH_PREFIX}{AUTH_SIGNUP}', json=invalid_login_data)
