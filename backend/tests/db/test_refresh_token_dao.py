...
async def test_save_token__valid_token__token_saved_and_returned(mocked_refresh_token_dao, db_user):
    db_token = await mocked_refresh_token_dao.create_token(db_user, REFRESH_TOKEN)
...
async def test_save_token__db_error__error_raised(mocked_session, db_user, mocked_refresh_token_dao):
    ...
    with pytest.raises(SQLAlchemyError) as exc:
        await mocked_refresh_token_dao.create_token(db_user, REFRESH_TOKEN)
...
async def test_revoke_token__token_exists__token_revoked(mocked_refresh_token_dao, db_user, db_refresh_token):
    new_token = await mocked_refresh_token_dao.create_token(db_user, REFRESH_TOKEN)

    revoked_token = await mocked_refresh_token_dao.revoke_token(db_refresh_token, revoked_by_id=new_token.id)
...
async def test_revoke_token__db_error__error_raised(
    ...
    with pytest.raises(SQLAlchemyError) as exc:
        await mocked_refresh_token_dao.revoke_token(db_refresh_token, revoked_by_id=None)
